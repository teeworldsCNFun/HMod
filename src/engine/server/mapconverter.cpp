...
CMapConverter::CMapConverter(IStorage *pStorage, IEngineMap *pMap, IConsole* pConsole) : CLuaClass("GameController") {
    m_pStorage = pStorage;
    m_pMap = pMap;
    m_pConsole = pConsole;
    m_pTiles = 0;
...
void CMapConverter::CopyMapInfo()
{
	...
	dbg_assert(Map()->IsLoaded(), "MapConverter:CreateMap() used but no map currently loaded");
	if(pItem)
