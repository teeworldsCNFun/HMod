...
void CGameContext::CreateExplosion(vec2 Pos, int Owner, int Weapon, bool NoDamage, float DamageMul, float ForceRadius, float DamageRadius, std::vector<CEntity*> *plNoDamage)
{
	...
	int Num = m_World.FindEntities(Pos, ForceRadius, (CEntity**)apEnts, MAX_CLIENTS, CGameWorld::ENTTYPE_CHARACTER);
	for(int i = 0; i < Num; i++)
	{
...
CPickup *CGameContext::CreateEntityPickup(int Type, int SubType, vec2 Pivot, vec2 RelPos, int PosEnv)
{
	return new CPickup(&m_World, Type, SubType, Pivot, RelPos, PosEnv);
}
